...
export async function getDatosConos() {
    try {
        const res = await fetch(`${URL_BASE}/items/light_cones?limit=-1`, { headers, cache: 'no-store' });
        const json = await res.json();
        return json.data || [];
    } catch (e) {
        console.error("Error fetching light cones:", e);
        return [];
    }
}